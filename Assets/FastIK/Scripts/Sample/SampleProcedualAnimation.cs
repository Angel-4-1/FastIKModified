...
namespace DitzelGames.FastIK
{
    class SampleProcedualAnimation :  MonoBehaviour
    {
        ...
        public void LateUpdate()
        {
            ...
            Step.Translate(Vector3.forward * Time.deltaTime * speed);
            if (Step.position.z > 1f)
                Step.position = Step.position + Vector3.forward * -4f;
            //Step2
            Step2.Translate(Vector3.forward * Time.deltaTime * speed);
            if (Step2.position.z > 1f)
                Step2.position = Step2.position + Vector3.forward * -4f;
